...
export const useExamTemplates = () => {
  ...
  const analyzeTemplate = useCallback(
    async (data: CreateExamTemplatePayload): Promise<string | null> => {
      setLoading(true);
      setError(null);
      try {
        const res = await ExamTemplateService.analyzeTemplate(data);
        return res.data; // Return analysis string directly
      } catch (err) {
        handleError(err, "Failed to analyze template");
        return null;
      } finally {
        setLoading(false);
      }
