...
const AIDashboardChatBubble: React.FC = () => {
        ...
        if (response) {
            setMessages((prev) => {
                ...
                if (lastMessage && lastMessage.sender === 'ai') {
                    return [...prev.slice(0, -1), { ...lastMessage, text: response }];
                } else {
                    return [...prev, { id: `ai-${Date.now()}`, text: response, sender: 'ai' }];
                }
            });
        }
    }, [response]);
    ...
    useEffect(() => {
        if (error) {
            setMessages((prev) => [
                ...prev,
                { id: `error-${Date.now()}`, text: `Xin lỗi, đã có lỗi xảy ra: ${error}`, sender: 'ai' }
            ]);
        }
    }, [error]);
    ...
    return (
        <>
            ...
            <button
                ...
                title={isOpen ? "Close chat" : "Ask AI about Dashboard"}
            >
                {isOpen ? (
            ...
            {!isOpen && (
                <div className="fixed bottom-6 right-6 z-40 w-14 h-14 rounded-full bg-gradient-to-br from-blue-500 via-purple-500 to-pink-500 animate-ping opacity-30" />
            )}
            ...
            {isOpen && (
                <div className="fixed bottom-24 right-6 z-50 w-[380px] h-[520px] bg-white rounded-2xl shadow-2xl border border-gray-200 flex flex-col overflow-hidden animate-fade-in">
                    {/* Header */}
                    <div className="bg-gradient-to-r from-blue-600 via-purple-600 to-pink-500 text-white p-4 flex items-center gap-3">
                        <div className="relative">
                        ...
                        <div className="flex-1">
                            <h3 className="font-semibold text-base">AI Dashboard Assistant</h3>
                            <p className="text-xs text-white/80">Ask me for your dashboard information.</p>
                        </div>
                        <button
                        ...
                        {messages.length === 0 && !isLoading && (
                            <div className="text-center py-8">
                                ...
                                <p className="text-sm text-gray-500 mb-4">
                                    I can help you understand the information on your dashboard.
                                </p>
                                <div className="space-y-2">
                                    {suggestedQuestions.map((question, index) => (
                                        <button
                                            ...
                                            className="block w-full text-left px-4 py-2.5 bg-white border border-gray-200 rounded-xl text-sm text-gray-700 hover:bg-purple-50 hover:border-purple-300 hover:text-purple-700 transition-all duration-200 shadow-sm"
                                        >
                                        ...
                                        {message.sender === 'ai' ? (
                                            <ReactMarkdown
                                                components={{
                                                    ...
                                                    code: ({ children }) => (
                                                        <code className="bg-gray-100 px-1.5 py-0.5 rounded text-xs font-mono text-purple-700">
                                                            {children}
                                                        </code>
                                                    ),
                                                }}
                    ...
                    <div className="p-4 bg-white border-t border-gray-200">
                        <form onSubmit={handleSubmit} className="flex gap-2">
                            <input
                                ...
                                className="flex-1 px-4 py-2.5 rounded-full border border-gray-300 bg-gray-50 focus:outline-none focus:ring-2 focus:ring-purple-500 focus:border-transparent focus:bg-white transition-all text-sm placeholder-gray-400"
                                placeholder="Ask about your dashboard..."
                                disabled={isLoading}
                            />
                            <button
                                type="submit"
                                className="w-11 h-11 bg-gradient-to-r from-blue-500 to-purple-600 hover:from-blue-600 hover:to-purple-700 text-white rounded-full flex items-center justify-center transition-all duration-200 transform hover:scale-105 disabled:opacity-50 disabled:cursor-not-allowed disabled:transform-none shadow-lg"
...
export default AIDashboardChatBubble;
